...
class RetrievalStrategyManager:
    ...
    async def retrieve(
        ...
        **kwargs
        ...
        if not use_case:
            ...
            chat_logger.info(f"Auto-detected use case: {use_case}", 
    ...
    async def _chat_strategy(
        ...
        try:
            ...
            # Build context
            context = RetrievalStrategyManager._build_context(reranked, style='conversational')
    ...
    async def _evaluation_strategy(
        ...
        try:
            ...
            chat_logger.info(f"EVALUATION strategy retrieved {len(expanded_results)} chunks (with expansion)")
    ...
    async def _qa_generation_strategy(
        ...
        """
        chat_logger.info("Using QA_GENERATION retrieval strategy", num_questions=num_questions)
        ...
        try:
            ...
            results_sorted = sorted(
                results, 
                key=lambda x: x.get('metadata', {}).get('sequential_id', x.get('chunk_index', 0))
            )
            ...
            chat_logger.info(f"QA_GENERATION strategy retrieved {len(sequential_results)} sequential chunks")
            ...
            return {
                ...
                "strategy": "qa_generation",
                "filtered_by_chapter": query_metadata['chapter']['value'] if query_metadata['chapter']['confidence'] > 0.70 else None,
    ...
    async def _notes_strategy(
        ...
        """
        chat_logger.info("Using NOTES retrieval strategy", max_chunks=max_chunks)
        ...
        try:
            ...
            chat_logger.info(f"NOTES strategy retrieved {len(results_sorted)} chunks in {len(grouped)} sections")
    ...
    @staticmethod
    def _simple_rerank(chunks: List[Dict[str, Any]], query: str, top_k: int) -> List[Dict[str, Any]]:
    ...
    async def _expand_context(
        ...
        for chunk in chunks:
            sequential_id = chunk.get('metadata', {}).get('sequential_id', chunk.get('chunk_index'))
    ...
    @staticmethod
    def _group_by_sections(chunks: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
