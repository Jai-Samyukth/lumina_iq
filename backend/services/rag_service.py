...
class RAGService:
    ...
    async def index_document(
        ...
        """
        try:
            chat_logger.info("Starting document indexing", 
                           filename=filename, 
            ...
            if file_path:
                ...
                file_hash = file_hash_service.calculate_content_hash(content.encode('utf-8'))
            
            if not file_hash:
                chat_logger.warning("Could not calculate file hash, proceeding without duplicate check")
            
            # Check if this exact file was already uploaded by this user
            if file_hash:
                existing_doc = document_tracking_service.check_document_exists(token, file_hash)
                if existing_doc:
                    chat_logger.info("Duplicate document detected", 
                                   filename=filename,
            ...
            if is_indexed:
                chat_logger.info("Document already indexed in Qdrant", filename=filename)
                # Still add to tracking if not there
                if file_hash:
                    document_tracking_service.add_document(token, filename, file_hash, file_size)
                return {
            ...
            chat_logger.info(f"Created {len(chunks_text)} chunks with rich metadata", filename=filename)
            
            # Generate embeddings for all chunks
            embeddings = await EmbeddingService.generate_embeddings_batch(chunks_text)
            
            chat_logger.info(f"Generated {len(embeddings)} embeddings", filename=filename)
            ...
            if file_hash:
                ...
                chat_logger.info("Document added to tracking database", 
                               filename=filename,
                               hash=file_hash[:16])
            
            # Extract metadata statistics
            chapters = set(m.get('chapter_number') for m in metadata_list if m.get('chapter_number'))
            sections = set(m.get('section_number') for m in metadata_list if m.get('section_number'))
            ...
            chat_logger.error("Failed to index document", filename=filename, error=str(e))
            return {
                "status": "error",
                "filename": filename,
    ...
    async def retrieve_context(
        ...
        """
        try:
            chat_logger.info("Retrieving context with ADVANCED RAG", 
            ...
            chat_logger.info(f"Advanced retrieval completed", 
            ...
            import traceback
            chat_logger.error("Failed to retrieve context", 
                            query=query[:100], 
                            error=str(e),
    ...
    async def delete_document_index(filename: str, token: str):
        ...
        try:
            ...
            return {
                "status": "success",
                "message": f"Deleted index for {filename}"
            }
        except Exception as e:
            chat_logger.error("Failed to delete document index", filename=filename, error=str(e))
            return {
                "status": "error",
    ...
    async def check_indexing_status(filename: str, token: str) -> bool:
        ...
        try:
            ...
            chat_logger.error("Failed to check indexing status", filename=filename, error=str(e))
            return False
