...
from qdrant_client.models import (
    ...
    MatchValue, PayloadSchemaType, MatchAny, Range
)
...
class QdrantService:
            ...
            if self.collection_name not in collection_names:
                self.client.create_collection(
                    ...
                    vectors_config=VectorParams(
                        size=768,  # Gemini embedding-001 produces 768-dimensional vectors
    ...
    async def index_document(
        ...
        """Index document chunks with their embeddings and rich metadata"""
        try:
            points = []
            ...
            chat_logger.info(f"Indexed {len(points)} chunks for {filename} with metadata")
            return len(points)
        except Exception as e:
        ...
        try:
            ...
            if metadata_filters:
                for meta_filter in metadata_filters:
                    ...
                    filter_type = meta_filter.get('type', 'match')  # 'match', 'range', 'any'
            ...
            chat_logger.info(f"Found {len(results)} similar chunks", token=token, filters=len(metadata_filters) if metadata_filters else 0)
            return results
        except Exception as e:
    ...
    async def delete_document_chunks(self, filename: str, token: str):
        ...
        try:
            ...
            chat_logger.error("Failed to delete document chunks", filename=filename, error=str(e))
            raise
    ...
    async def check_document_indexed(self, filename: str, token: str) -> bool:
        ...
        try:
            ...
            chat_logger.error("Failed to check if document is indexed", filename=filename, error=str(e))
            return False
